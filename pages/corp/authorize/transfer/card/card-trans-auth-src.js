				 ...
				 requestMBServiceCorp.post(data, function(data) {
					 ...
					 if (resp.respCode === '0' && resp.respJsonObj.makers.length > 0 && resp.respJsonObj.limit) {
						 ...
						 if (resp.respCode == '0' && resp.respJsonObj.list_pending.length > 0) {
							 ...
							 var docXsl = getCachePageXsl("corp/authorize/transfer/card/card-trans-auth-result-tbl");
...
function handleSelectTransType(e) {
	if (e.selectedValue2 === 'T19'){
		...
		navCachedPages['corp/authorize/transfer/account/account-trans-auth-src'] = null;
...
function showDetailTransaction(idx) {
    ...
    gCorp.cmdType = CONSTANTS.get("CMD_CO_AUTHORIZE_CARD_TRANSFER");
...
function genReviewXML(transInfo) {
    ...
    valueNode = createXMLNode('value', CONST_STR.get('TRANS_CARD_TYPE'), docXml, rowNode);
...
function authorizeTransaction() {
	...
	if (listSelectedTrans.length > 0){
		...
		// if (totalAmount > parseInt(listSelectedTrans[i].SO_DU_KHA_DUNG)){
		// 	showAlertText(CONST_STR.get('TOPUP_EXCEED_AVAIL_BALANCE'));
		// 	return;xx
		// }
	...
	for(var i in listSelectedTrans) {
		...
		transInfo.push({
			...
			chargeincl : tran.NGUOI_CHIU_PHI,
		});
		...
		for (var j in transSummaryArr) {
			...
			if (tmpTran.account == tran.TK_CHUYEN) {
				if(tran.NGUOI_CHIU_PHI == "N"){
					tmpTran.totalAmount += (parseInt(tran.SO_TIEN) + parseInt(tran.PHI_DV));
				} else {
					tmpTran.totalAmount += parseInt(tran.SO_TIEN);
				}
				isExist = true;
			}
		}
		...
		if(tran.NGUOI_CHIU_PHI == "N"){
			amount = parseInt(tran.SO_TIEN) + parseInt(tran.PHI_DV);
		} else {
			amount = parseInt(tran.SO_TIEN);
		}
