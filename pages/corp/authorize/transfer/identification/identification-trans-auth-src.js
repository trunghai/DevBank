...
function handleSearchInput(e) {
    if (currentPage == "corp/authorize/transfer/identification/identification-trans-auth-src") {
...
function getListMakers() {
    ...
    angular.module("EbankApp").controller("identification-trans-auth-src", function ($scope, requestMBServiceCorp) {
        if (!gTrans.backFromOther){
            ...
            var gprsCmd = new GprsCmdObj(CONSTANTS.get("CMD_CO_AUTHORIZE_IDENTIFICATION"), "", "", gUserInfo.lang, gUserInfo.sessionID, arrayArgs);
            gprsCmd.raw = '';
...
//&& (parseInt(response.responseType) == parseInt(CONSTANTS.get('CMD_CO_AUTHORIZE_IDENTIFICATION')))
...
function viewSearchResults() {
    ...
    var xslDoc = getCachePageXsl("corp/authorize/transfer/identification/transfer-identification-search-result");
...
function genXMLListTrans(pJson) {
    ...
    for (var i = 0; i < pJson.length; i++) {
        ...
        nodeChild = createXMLNode('passport', pJson[i].PASSPORT, docXml, nodeInfo);
        nodeChild = createXMLNode('amount', formatNumberToCurrency(pJson[i].NUMAMOUNT), docXml, nodeInfo);
        nodeChild = createXMLNode('beneName', pJson[i].TEN_NGUOI_NHAN, docXml, nodeInfo);
...
function requestDetailSuccess(e) {
    ...
    if (checkResponseCodeSuccess(response.respCode) && (parseInt(response.responseType) == parseInt(CONSTANTS.get("CMD_CO_AUTHORIZE_IDENTIFICATION")))) {
...
function genDetailScreen(transInfo) {
    ...
    valueNode = createXMLNode('value', CONST_STR.get('IDENTIFICATION_TRANS_TYPE'), docXml, rowNode);
    ...
    valueNode = createXMLNode('value', transInfo.NGAY_CAP, docXml, rowNode);
	
	// noi cap
    rowNode = createXMLNode('row', '', docXml, sectionNode);
    labelNode = createXMLNode('label', CONST_STR.get('IDENTIFICATION_PLACE'), docXml, rowNode);
    valueNode = createXMLNode('value', transInfo.NOI_CAP, docXml, rowNode);
    ...
    valueNode = createXMLNode('value', transInfo.PHONE, docXml, rowNode);
   ...
   else
   {
	...
	var soDuSauChuyen;
	if(transInfo.CHARGEINCL == "N") {
		soDuSauChuyen = transInfo.BALANCE_BEFOR - transInfo.NUMAMOUNT - transInfo.CHARGEFORDOM;
	}
	if(transInfo.CHARGEINCL == "Y"){
		soDuSauChuyen = transInfo.BALANCE_BEFOR - transInfo.NUMAMOUNT;
	}
	
    rowNode = createXMLNode('row', '', docXml, sectionNode);
    labelNode = createXMLNode('label', CONST_STR.get('TRANS_LOCAL_BALANCE_CONT'), docXml, rowNode);
    valueNode = createXMLNode('value', formatNumberToCurrency(soDuSauChuyen) + " " + transInfo.CODTRNCURR, docXml, rowNode);
    ...
    if (!viewBackDetail) {
        ...
        gCorp.cmdType = CONSTANTS.get("CMD_CO_AUTHORIZE_IDENTIFICATION");
...
function approveTransaction(isApprove) {
    ...
    if (listTrans.length <= 0) {
        ...
        if (listTrans.length > 0){
            var totalAmount = 0;
            var listObj = [];
            for (var i in listTrans){
                if (listObj.length == 0){
                    var obj = {};
                    obj.balance = listTrans[i].BALANCE_BEFOR;
                    obj.srcAccount = listTrans[i].IDSRCACCT;
                    obj.totalAmount = 0;
                    listObj.push(obj);
                }else {
                    for (var j in listObj){
                        if (listTrans[i].IDSRCACCT == listObj[j].srcAccount){
                            break;
                        }else {
                            if(j == listObj.length){
                                var obj = {};
                                obj.balance = parseInt(listTrans[i].BALANCE_BEFOR);
                                obj.srcAccount = listTrans[i].IDSRCACCT;
                                obj.totalAmount = 0;
                                listObj.push(obj);
                                break;
                ...
                if (listTrans[i].CHARGEINCL === 'N'){
                    totalAmount += parseInt(listTrans[i].NUMAMOUNT) + parseInt(listTrans[i].CHARGEFORDOM);
                }else {
                    totalAmount += parseInt(listTrans[i].NUMAMOUNT);

                }

                for (var x in listObj){
                    if (listObj[x].srcAccount == listTrans[i].IDSRCACCT){
...
function genMulTransScreen(respJsonObj, isApprove, request) {
    ...
    for (var i = 0; i < respJsonObj.length; i++) {
        ...
        tdNode = createXMLNode('td', respJsonObj[i].PASSPORT, docXml, trNode);
        createXMLNode('title', CONST_STR.get("IDENTIFICATION_AUTHORIZE_ID"), docXml, tdNode);
        tdNode = createXMLNode('td', respJsonObj[i].TEN_NGUOI_NHAN, docXml, trNode);
        createXMLNode('title', CONST_STR.get("COM_RECEIVER"), docXml, tdNode);
        tdNode = createXMLNode('td', formatNumberToCurrency(respJsonObj[i].NUMAMOUNT) + " VND", docXml, trNode);
