...
function loadInitData() {
	angular.module("EbankApp").controller("account-trans-auth-src", function($rootScope, $scope, requestMBServiceCorp) {
		// body...
		if (!gTrans.isBack) {
			...
			requestMBServiceCorp.post(data, function(data) {
				...
				if (resp.respCode == 0 && resp.respJsonObj.makers.length > 0 && resp.respJsonObj.limit) {
					...
					if (resp.respJsonObj.list_pending.length > 0) {
						...
						var docXsl = getCachePageXsl("corp/authorize/transfer/account/account-trans-auth-result-tbl");
...
{
	var cbxValues = (gUserInfo.lang == 'EN')? ['Transfer to card number', 'Transfer to account number'] : ['Chuyển tiền nhanh qua số thẻ', 'Chuyển tiền nhanh qua số tài khoản'];
	addEventListenerToCombobox(handleSelectTransTypeAcc, handleCloseTransTypeCbx);
	showDialogList(CONST_STR.get('COM_CHOOSEN_TYPE_TRANS'), cbxValues, ['T19', 'T21'], false);
}

function handleSelectTransTypeAcc(e) {
	if (e.selectedValue2 === 'T21'){
		removeEventListenerToCombobox(handleSelectTransTypeAcc, handleCloseTransTypeCbx);
		searchInfo.transType = e.selectedValue2;
		document.getElementById('id.trans-type').value = e.selectedValue1;
	}else {
		navCachedPages['corp/authorize/transfer/card/card-trans-auth-src'] = null;
...
function requestMBServiceSuccess(e){
	...
	if (resp.respCode == '0' && resp.respJsonObj.list_pending.length > 0) {
		results = resp.respJsonObj.list_pending;
		totalPages = getTotalPages(results.length);
...
function genXmlData(idx) {
	...
	for (var i = startIdx; i < endIdx; i++) {
		...
		if (typeof obj !== "undefined") {
			...
			createXMLNode('acccontent4', obj.BENNAME, docXml,childNodeCont);
...
function showDetailTransaction(idx) {
	...
	//kiểm tra số tiền chuyển với số dư khả dụng
	if (curTrans.NGUOI_CHIU_PHI === 'N'){
		var totalAmount = parseInt(curTrans.SO_TIEN) + parseInt(curTrans.PHI_DV);
		if (totalAmount > parseInt(curTrans.SO_DU_KHA_DUNG)){
			showAlertText(CONST_STR.get('TOPUP_EXCEED_AVAIL_BALANCE'));
			return;
		}
	}

	if (curTrans.NGUOI_CHIU_PHI === 'Y'){
		var totalAmount = parseInt(curTrans.SO_TIEN) - parseInt(curTrans.PHI_DV);
		if (totalAmount > parseInt(curTrans.SO_DU_KHA_DUNG)){
			showAlertText(CONST_STR.get('TOPUP_EXCEED_AVAIL_BALANCE'));
			return;
    ...
    gCorp.cmdType = CONSTANTS.get("CMD_CO_AUTHORIZE_ACCOUNT");
...
function genReviewXML(transInfo) {
    ...
    valueNode = createXMLNode('value', CONST_STR.get('TRANS_ACC_TRANSTYPE'), docXml, rowNode);
	...
	rowNode = createXMLNode('row', '', docXml, sectionNode);
    labelNode = createXMLNode('label', CONST_STR.get('COM_RECEIVER_BANK_NAME'), docXml, rowNode);
    valueNode = createXMLNode('value', transInfo.TEN_NGAN_HANG, docXml, rowNode);
	...
	var fee_payer = "";
	if(transInfo.NGUOI_CHIU_PHI == "N") {
		fee_payer = CONST_STR.get('IDENTIFICATION_FEE_SENDER');
	}
	if(transInfo.NGUOI_CHIU_PHI == "Y"){
		fee_payer = CONST_STR.get('IDENTIFICATION_FEE_RECEIVER_PAY');
	}
	// nguoi chiu phi 
	 rowNode = createXMLNode('row', '', docXml, sectionNode);
    labelNode = createXMLNode('label', CONST_STR.get('TRANSFER_FEE_PAYER'), docXml, rowNode);
    valueNode = createXMLNode('value', fee_payer, docXml, rowNode);
	...
	var soDuSauChuyen;
	if(transInfo.NGUOI_CHIU_PHI == "N") {
		soDuSauChuyen = soDuKhaDung - soTien - phidv;
	}
	if(transInfo.NGUOI_CHIU_PHI == "Y"){
		soDuSauChuyen = soDuKhaDung - soTien;
	}
    rowNode = createXMLNode('row', '', docXml, sectionNode);
...
function authorizeTransaction() {
	...
	if (listSelectedTrans.length > 0){
		...
		for (var i in listSelectedTrans){
			if (listObj.length == 0){
				var obj = {};
				obj.balance = listSelectedTrans[i].SO_DU_KHA_DUNG;
				obj.srcAccount = listSelectedTrans[i].TK_CHUYEN;
				obj.totalAmount = 0;
				listObj.push(obj);
			}else {
				for (var j in listObj){
					if (listTrans[i].TK_CHUYEN == listObj[j].srcAccount){
						break;
					}else {
						if(j == listObj.length){
							var obj = {};
							obj.balance = parseInt(listSelectedTrans[i].SO_DU_KHA_DUNG);
							obj.srcAccount = listSelectedTrans[i].TK_CHUYEN;
							obj.totalAmount = 0;
							listObj.push(obj);
							break;
			...
			if (listSelectedTrans[i].NGUOI_CHIU_PHI === 'N'){
				totalAmount += parseInt(listSelectedTrans[i].SO_TIEN) + parseInt(listSelectedTrans[i].PHI_DV);
			}else {
				totalAmount += parseInt(listSelectedTrans[i].SO_TIEN);

			}

			for (var x in listObj){
				if (listObj[x].srcAccount == listSelectedTrans[i].TK_CHUYEN){
					listObj[x].totalAmount = totalAmount;
					if (listObj[x].totalAmount > listObj[x].balance){
						showAlertText(CONST_STR.get('TOPUP_EXCEED_AVAIL_BALANCE'));
						return;
					}
				}
...
function genReviewTableXML(action) {
	...
	for(var i in listSelectedTrans) {
	    ...
	    tdNode = createXMLNode("td", tran.BENNAME, xmlDoc, trNode);
