            ...
            <div class="panelContent">
              <table width='100%' align='center'>
                ...
                <tr>
                  <td>
                    <table>
                      <xsl:if test="count(button)=1">
                        <xsl:attribute name="class">button-group button-group-1</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="count(button)=2">
                        <xsl:attribute name="class">button-group button-group-2</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="count(button)=3">
                        <xsl:attribute name="class">button-group button-group-3</xsl:attribute>
                      </xsl:if>
                      <tr>
      ...
      <tr>
        <td>
          <div class="input-group" style="margin-top: 15px">
            <input maxlength="250" id="reject-reason" type="text" class="form-control" placeholder="{.}" />
          </div>
        </td>
      </tr>
    </xsl:template>
    ...
    <xsl:template match="button">
      <td>
        ...
        <xsl:if test="type='back'">
          <input type="button" class='btnshadow btn-primary' onclick='onBackClick()' value='{label}' />
        </xsl:if>
